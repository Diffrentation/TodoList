...
import {
  ...
  MoreHorizontal, PanelLeft, PanelRight, Plus, Search, Send, Settings, SlidersHorizontal, Sun, Tag, UserRound, Users, X, LockKeyhole, LockOpen,
} from "lucide-react";
...
function FieldMenu({ view, setView, fields, setFields, priority, setPriority, page, close }) {
  ...
  return <div ref={menuRef} className="absolute right-0 top-11 z-30 w-64 rounded-xl border border-slate-200 bg-white p-2 shadow-xl dark:border-slate-700 dark:bg-slate-900" role="menu">
    ...
    {visibleKeys.map((key) => { const Icon = fieldMeta[key]?.icon; return <div key={key} className="relative flex items-center justify-between rounded-md px-2 py-2 text-sm hover:bg-slate-50 dark:hover:bg-slate-800">
      ...
      {key === "priority" && priorityFilterOpen && <>
        ...
        <div onClick={(event) => event.stopPropagation()} className="absolute left-full top-0 z-40 ml-1 w-40 rounded-lg border border-slate-200 bg-white p-1 shadow-xl dark:border-slate-700 dark:bg-slate-900">
...
function NewItemDialog({ kind, initial, onClose, onSubmit, projects, team, user }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4" role="presentation" onMouseDown={onClose}>
    <form onSubmit={submit} onMouseDown={(event) => event.stopPropagation()} className="max-h-[calc(100dvh-2rem)] w-full max-w-xl overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl dark:bg-slate-900 sm:p-6" role="dialog" aria-modal="true" aria-labelledby="new-item-title">
      <div className="mb-5 flex items-center justify-between"><h2 id="new-item-title" className="text-lg font-semibold">{isEdit ? `Edit ${kind}` : `Create ${kind}`}</h2><IconButton label="Close dialog" onClick={onClose}><X size={18}/></IconButton></div>
      ...
      {kind === "task" && <>
        ...
        {isTeamTask && <fieldset className="mb-5 rounded-xl border border-slate-200 p-3 dark:border-slate-700"><legend className="px-1 text-sm font-medium">Assignees</legend><p className="mb-2 text-xs text-slate-500 dark:text-slate-400">Leave this unchanged to assign project members automatically, or choose people below.</p><div className="grid gap-1 sm:grid-cols-2">{teamMembers.map((member) => { const id = String(member._id || member.id); return <label key={id} className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-800"><input type="checkbox" checked={assignees.includes(id)} onChange={() => toggleAssignee(id)} className="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"/><Avatar user={member} size="sm"/><span className="truncate text-sm">{member.firstname} {member.lastname}</span></label>; })}</div></fieldset>}
        {isTeamTask && <label className="mb-5 flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 p-3 text-sm dark:border-slate-700"><input type="checkbox" checked={isPrivate} onChange={(event) => setIsPrivate(event.target.checked)} className="mt-0.5 h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"/><span><span className="flex items-center gap-1.5 font-medium"><LockKeyhole size={15}/>Private task</span><span className="mt-0.5 block text-xs leading-5 text-slate-500 dark:text-slate-400">Only assignees, project members, and the reporter can view it.</span></span></label>}
      </>}
      {kind === "project" && <>
...
function TeamSettings({ team, onClose, onCreate, onRename, onInvite, onRemoveMember, onDeleteTeam, currentUserId }) {
  ...
  const submitInvite = async (event) => {
    ...
    setSaving(true);
    try { await onInvite(team._id, email.trim()); setEmail(""); setInviteOpen(false); } finally { setSaving(false); }
  };
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4" role="presentation" onMouseDown={onClose}>
    <div onMouseDown={(event) => event.stopPropagation()} className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl dark:bg-slate-900" role="dialog" aria-modal="true" aria-labelledby="team-settings-title">
      <div className="mb-5 flex items-center justify-between"><h2 id="team-settings-title" className="text-lg font-semibold">{isCreate ? "New team" : "Team settings"}</h2><IconButton label="Close" onClick={onClose}><X size={18}/></IconButton></div>
...
const TaskCard = memo(function TaskCard({ task, fields, onOpen, onDelete }) {
  const [menuOpen, setMenuOpen] = useState(false);
  const assignee = task.assignees?.[0] || task.reporter;
  return <article draggable onDragStart={(event) => event.dataTransfer.setData("text/task-id", task._id)} onClick={() => onOpen(task)} className="cursor-pointer rounded-2xl border border-slate-200 bg-white p-3.5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-700 dark:bg-slate-900">
    <div className="flex items-start gap-2">
      ...
      <div className="relative -mr-1.5 -mt-1 shrink-0">
        ...
        {menuOpen && <>
          ...
          <div onClick={(event) => event.stopPropagation()} className="absolute right-0 top-8 z-40 w-36 rounded-lg border border-slate-200 bg-white p-1 shadow-xl dark:border-slate-700 dark:bg-slate-900">
            <button onClick={() => { setMenuOpen(false); onOpen(task); }} className="flex w-full items-center rounded-md px-2.5 py-1.5 text-left text-xs font-medium hover:bg-slate-100 dark:hover:bg-slate-800">Edit task</button>
...
const TaskBoard = memo(function TaskBoard({ tasks, fields, onOpen, onMove, onNew, onDelete }) {
  ...
  return <div className="grid min-w-[900px] grid-cols-4 gap-4" aria-label="Task board">
    {columns.map((column) => {
      const items = tasks.filter((task) => displayStatus(task.status) === column.id);
      const isCollapsed = collapsed[column.id];
      return <section key={column.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { const id = event.dataTransfer.getData("text/task-id"); if (id) onMove(id, column.id); }} className="min-h-72 rounded-xl bg-slate-100/80 p-2 dark:bg-slate-900/70">
  ...
  return <div className="space-y-5">
    {columns.map((column) => {
      ...
      return <section key={column.id}>
        ...
        {!isCollapsed && <div className="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700">
...
function DatePicker({ value, onChange }) {
  ...
  return <div className="relative">
    ...
    {open && <>
      <div className="fixed inset-0 z-30" onClick={() => setOpen(false)}/>
      <div className="absolute right-0 top-8 z-40 w-64 rounded-xl border border-slate-200 bg-white p-3 shadow-xl dark:border-slate-700 dark:bg-slate-900">
...
function TaskPanel({ task, tasks, teams, user, onClose, onSave, onDelete, onOpen, onCreateSubtask, onAddComment, onToggleWatch }) {
  ...
  return <div className="fixed inset-0 z-40 flex items-stretch justify-center bg-slate-950/40 sm:items-center sm:p-6" role="presentation" onMouseDown={onClose}>
    <div onMouseDown={(event) => event.stopPropagation()} className="flex h-full w-full max-w-5xl overflow-hidden bg-white shadow-2xl dark:bg-slate-950 sm:h-[88vh] sm:rounded-2xl" role="dialog" aria-modal="true" aria-labelledby="task-detail-title">
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 shrink-0 items-center justify-between border-b border-slate-200 px-5 dark:border-slate-800">
          ...
          <div className="flex items-center gap-1">
            {taskTeamId && <IconButton label={draft.private ? "Private to assignees — click to share with the whole team" : "Shared with the whole team — click to make private"} onClick={togglePrivate} className={draft.private ? "bg-amber-50 text-amber-700 hover:bg-amber-100 dark:bg-amber-950/30 dark:text-amber-400 dark:hover:bg-amber-950/50" : "text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400"}>{draft.private ? <LockKeyhole size={17}/> : <LockOpen size={18}/>}</IconButton>}
            ...
            <IconButton label="Close task details" onClick={onClose}><X size={19}/></IconButton>
          </div>
        </header>
        <div className="flex-1 overflow-y-auto p-5 sm:p-7">
          <input id="task-detail-title" value={draft.title} onChange={(event) => update("title", event.target.value)} maxLength={200} className="w-full border-0 bg-transparent p-0 text-2xl font-semibold outline-none focus:ring-0"/>
          <textarea value={draft.description || ""} onChange={(event) => update("description", event.target.value)} maxLength={4000} className="mt-2 min-h-16 w-full resize-none border-0 bg-transparent p-0 text-sm leading-6 text-slate-600 outline-none focus:ring-0 dark:text-slate-300" placeholder="Add a description…"/>
...
export default function WorkspaceApp() {
  ...
  const shownTasks = useMemo(() => tasks.filter((task) => { if (task.parentTask) return false; const teamId = task.team?._id || task.team || null; const inScope = activeTeamId ? String(teamId) === String(activeTeamId) : !teamId; if (!inScope) return false; const haystack = `${task.title} ${task.description || ""} ${(task.labels || []).join(" ")}`.toLowerCase(); return (!query || haystack.includes(query.toLowerCase())) && (priority === "all" || task.priority === priority); }), [tasks, query, priority, activeTeamId]);
  const shownProjects = useMemo(() => projects.filter((project) => { const teamId = project.team?._id || project.team || null; const inScope = activeTeamId ? String(teamId) === String(activeTeamId) : !teamId; return inScope && project.title.toLowerCase().includes(query.toLowerCase()) && (priority === "all" || project.priority === priority); }), [projects, query, priority, activeTeamId]);
  ...
  const inviteMember = async (id, email) => { try { const response = await api.post(`/teams/${id}/members`, { email }); if (response.data.team) setTeams((current) => current.map((item) => item?._id === id ? response.data.team : item).filter(Boolean)); toast.success(response.data.message || "Invitation sent"); } catch (error) { toast.error(error.response?.data?.message || "Unable to send invitation"); } };
